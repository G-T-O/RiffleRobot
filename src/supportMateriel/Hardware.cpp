...
Hardware::Hardware() {
    pwmgauche.period_us(800);
    pwmdroite.period_us(800);
...
void Hardware::move() {
    dirgauche=0;
    dirdroite=1;
    pwmgauche.pulsewidth_us(590);
    pwmdroite.pulsewidth_us(590);
    return;
}
...
void Hardware::stepTurn(int dir) {
    if (dir==RIGHT) {
        ...
        pwmdroite.pulsewidth_us(445);
//    } else {
//        dirgauche=0;
//        dirdroite=1;
//        pwmgauche.pulsewidth_us(0);
//        pwmdroite.pulsewidth_us(300);
    }
    return;
}

void Hardware::Turn(int dir ,int left,int right) {
    if (dir==RIGHT) {
        dirgauche=0;
        dirdroite=0;
        pwmgauche.pulsewidth_us(left);
        pwmdroite.pulsewidth_us(right);
    } else {
        dirgauche=1;
        dirdroite=1;
        pwmgauche.pulsewidth_us(left);
